...
export function CronPage() {
  ...
  const handleDelete = async (job: CronJobRecord) => {
    const ok = await deleteCronJob(job.id);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div ref={headerRef} className="space-y-2">
        <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
          ...
          <span className="text-sm font-medium">{t('cron.title')}</span>
        </div>
        <h1 className="font-display text-2xl font-bold text-[var(--text-primary)]">{t('cron.title')}</h1>
        <p className="text-[var(--text-muted)]">{t('cron.subtitle')}</p>
      </div>
      ...
      <Card className="space-y-4 border-amber-100 dark:border-amber-900/40 bg-gradient-to-br from-amber-50/50 to-orange-50/30 dark:from-amber-950/20 dark:to-orange-950/10">
        ...
        {proposal && (
          <Card className="bg-[var(--bg-card)] border-emerald-200 dark:border-emerald-800 space-y-3">
            ...
            <div>
              <p className="text-xs text-[var(--text-muted)]">{t('cron.previewMessage')}</p>
              <p className="text-sm mt-1 whitespace-pre-wrap">{proposal.message}</p>
            </div>
            <select
            ...
            <div className="flex gap-2">
              <Button onClick={() => void handleConfirmCreate()} disabled={submitting}>
                {t('cron.confirmCreate')}
              </Button>
      ...
      {showAdvanced && (
        <Card className="space-y-4">
          ...
          />
          <input
            className="w-full rounded-xl border border-[var(--border)] px-3 py-2 text-sm font-mono bg-[var(--bg-card)]"
            placeholder={t('cron.schedulePlaceholder')}
            value={schedule}
            onChange={(e) => setSchedule(e.target.value)}
          />
          <p className="text-xs text-[var(--text-muted)]">{t('cron.scheduleHint')}</p>
          <textarea
            className="w-full rounded-xl border border-[var(--border)] px-3 py-2 text-sm bg-[var(--bg-card)] min-h-[80px]"
          ...
          <Button onClick={() => void handleAdvancedCreate()} disabled={submitting || !name.trim()}>
            {t('cron.create')}
          </Button>
        </Card>
      )}
          ...
          {jobs.map((job) => {
            ...
            return (
              <Card key={job.id} className="p-4 space-y-3 hover:shadow-md transition-shadow">
                ...
                <p className="text-sm text-[var(--text-primary)] pl-[52px]">{job.message}</p>
                <div className="flex flex-wrap items-center gap-4 text-xs text-[var(--text-muted)] pl-[52px]">
                  {agent && (
                    <span className="inline-flex items-center gap-1.5">
                      {t('cron.agent')}:
                      <ColleagueAvatar agentId={agent.id} name={agent.name} size="sm" />
                      {agent.name}
                    </span>
                  )}
